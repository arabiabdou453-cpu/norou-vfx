...
$checks = [ordered]@{
    ...
    'Desktop navigation repairs a hidden Framer anchor before scrolling' = $html -match 'const isDesktopNavigation\s*=\s*\(\)\s*=>\s*window\.matchMedia\("\(min-width:\s*810px\)"\)\.matches' -and $html -match 'if \(isDesktopNavigation\(\) && !isUsableSectionTarget\(target\)\) \{\s*improveNavigation\(\);\s*target = document\.getElementById\(id\);'
