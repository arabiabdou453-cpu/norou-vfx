...
$checks = [ordered]@{
    ...
    'Section navigation jumps immediately below the navbar' = $html -match 'enableSectionNavigation' -and $html -match 'getBoundingClientRect\(\)\.top - offset' -and $html -match 'window\.scrollBy\(\{ top: delta, behavior: "auto" \}\)' -and $html -notmatch 'behavior:\s*window\.matchMedia'
    'Section navigation performs one immediate jump per click' = $html -match 'const alignAfterLayout' -and $html -notmatch 'settleDesktopNavigation\(target, offset\)'
