...
$checks = [ordered]@{
    ...
    'Section navigation moves smoothly below the navbar' = $html -match 'enableSectionNavigation' -and $html -match 'getBoundingClientRect\(\)\.top - offset' -and $html -match 'window\.scrollBy\(\{ top: delta, behavior: "smooth" \}\)' -and $html -notmatch 'behavior:\s*window\.matchMedia'
    'Section navigation corrects a post-layout offset' = $html -match 'const alignAfterLayout' -and $html -match 'settleDesktopNavigation\(target, offset\)'
    'Desktop navigation remains aligned through late Framer layout changes' = $html -match 'const settleDesktopNavigation' -and $html -match 'new ResizeObserver' -and $html -match 'layoutObserver\.observe\(document\.documentElement\)'
    'Desktop navigation repairs a hidden Framer anchor before scrolling' = $html -match 'const isDesktopNavigation\s*=\s*\(\)\s*=>\s*window\.matchMedia\("\(min-width:\s*810px\)"\)\.matches' -and $html -match 'if \(!isUsableSectionTarget\(target\)\) \{\s*improveNavigation\(\);\s*target = document\.getElementById\(id\);'
    'Desktop navigation captures the first click before Framer can intercept it' = $html -match 'window\.addEventListener\("click", \(event\) => \{\s*if \(isDesktopNavigation\(\)\) \{\s*navigateFromEvent\(event\);\s*return;\s*\}'
    'Every section link delegates first-click recovery to scrollToSection' = $html -match 'const link = source\.closest\(''a\[href\^="#"\]''\);\s*const id = link\?\.getAttribute\("href"\)\?\.slice\(1\);\s*if \(!id \|\| !sectionIds\.has\(id\)\) return;\s*if \(link\.closest\("\.norou-mobile-navigation"\)\)' -and $html -notmatch 'if \(!id \|\| !sectionIds\.has\(id\)\) return;\s*const target = document\.getElementById\(id\);\s*if \(!target\) return;'
    'Section navigation recognizes rendered targets without relying on offsetParent' = $html -match 'const isUsableSectionTarget\s*=\s*\(target\)\s*=>\s*\{[\s\S]*?getBoundingClientRect\(\)[\s\S]*?getComputedStyle\(target\)[\s\S]*?return rect\.width > 0 && rect\.height > 0 && style\.display !== "none" && style\.visibility !== "hidden";[\s\S]*?\};' -and $html -match 'if \(!isUsableSectionTarget\(target\)\)' -and $html -match 'if \(!isUsableSectionTarget\(target\)\) return false;'
    ...
    'Navigation recovery observes the content subtree without observing its own repairs' = $html -match 'new MutationObserver' -and $html -match 'getElementById\("main"\)' -and $html -match 'observer\.disconnect\(\);\s*improveNavigation\(\);\s*observer\.observe\(navigationHost'
    ...
    'Mobile uses a stable custom navbar' = $html -match '<nav\s+class="norou-mobile-navigation"\s+aria-label="Mobile navigation">' -and $html -match '\.norou-mobile-navigation\{display:none' -and $html -match '\.norou-mobile-navigation\{position:fixed' -and $headHtml -match '#main\s+nav[^{]*\{[^}]*display:\s*none\s*!important'
    'Mobile navbar toggle is accessible' = $html -match 'data-norou-mobile-nav-toggle' -and $html -match 'aria-controls="norou-mobile-menu"' -and $html -match 'aria-expanded="false"' -and $html -match 'setMobileNavigationOpen'
    'Mobile navbar links every primary section' = @('#home', '#work', '#services', '#about', '#contact') | ForEach-Object { $html -match ('class="norou-mobile-navigation__link"\s+href="' + [regex]::Escape($_) + '"') } | Where-Object { -not $_ } | Measure-Object | Select-Object -ExpandProperty Count | ForEach-Object { $_ -eq 0 }
    'Duplicate footer CTA is hidden before first paint' = $headHtml -match 'footer[^\{]*instagram\.com/norou\.vfx[^\{]*\{[^}]*display:\s*none\s*!important'
    'Mobile content is visible without Framer hydration' = $headHtml -match '@media\s*\(max-width:\s*809\.98px\)[\s\S]*?#main\s+\[data-framer-appear-id\][^{]*\{[^}]*opacity:\s*1\s*!important[^}]*transform:\s*none\s*!important'
    'Mobile document remains vertically scrollable' = $html -match 'overflow-y:\s*auto\s*!important' -and $html -match 'height:\s*auto\s*!important'
    'Mobile layout is constrained to the viewport' = $html -match '--norou-mobile-gutter:\s*clamp\(' -and $html -match 'overflow-x:\s*clip\s*!important' -and $html -match 'max-width:\s*100vw\s*!important'
    'Mobile full-height sections use the dynamic viewport' = $html -match 'min-height:\s*100dvh\s*!important'
    'Showreel title respects mobile gutters' = $html -match 'Section - Showreel"\]\s+svg\s*\{[^}]*max-width:\s*calc\(100vw\s*-\s*\(2\s*\*\s*var\(--norou-mobile-gutter\)\)\)\s*!important'
    'Legacy mobile menu injector is removed' = $html -notmatch 'const mobileMenu\s*=\s*document\.querySelector'
