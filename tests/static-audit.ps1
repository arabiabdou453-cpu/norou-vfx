...
$checks = [ordered]@{
    ...
    'Section navigation moves smoothly below the navbar' = $html -match 'enableSectionNavigation' -and $html -match 'getBoundingClientRect\(\)\.top - offset' -and $html -match 'animateDesktopScrollTo\(window\.scrollY \+ delta\)' -and $html -notmatch 'behavior:\s*window\.matchMedia'
    'Desktop navigation uses a short single-frame animation' = $html -match 'const animateDesktopScrollTo' -and $html -match 'requestAnimationFrame\(step\)' -and $html -match 'duration = 240'
    'Section navigation uses one animation per click' = $html -match 'const alignAfterLayout' -and $html -match 'animateDesktopScrollTo\(window\.scrollY \+ delta\)' -and $html -notmatch 'settleDesktopNavigation\(target, offset\)'
    ...
    'Desktop navigation repairs a hidden Framer anchor before scrolling' = $html -match 'const isDesktopNavigation\s*=\s*\(\)\s*=>\s*window\.matchMedia\("\(min-width:\s*810px\)"\)\.matches' -and $html -match 'if \(isDesktopNavigation\(\) && \(!target \|\| target\.offsetParent === null\)\) \{\s*improveNavigation\(\);\s*target = document\.getElementById\(id\);'
